...
Finally, my research design is cross-sectional, since I will be using reported rates of violence against women and an analysis of existing laws from one point in time.  Additionally, I did not interfere, but rather observed the existing data without any manipulation to deduce if there was any correlation between the existence of gender ministries and the rates of violence against women, when physically controlling for the country's laws and whether they specifically harm women.  Below is a plot (Figure 1) which summarizes the dependent variable.
